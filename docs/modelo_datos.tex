...
El modelo define 5 etiquetas de nodos principales, cada una con al menos 5 propiedades:
...
Se han definido 10 tipos de relaciones diferentes, con un mínimo de 3 propiedades cada una:
...
LIKED & User → Movie & fecha, motivación, intensidad \\
BOOKMARKED & User → Movie & fecha, prioridad, recordatorio \\
HAS\_GENRE & Movie → Genre & es\_principal, peso, origen \\
STARS\_IN & Actor → Movie & rol, orden, pantalla\_time \\
DIRECTED\_BY & Director → Movie & año\_filmacion, versión, credito\_principal \\
WROTE\_REVIEW & User → Movie & fecha, editado, spoiler \\
FOLLOWS & User → User & fecha, notificaciones, nivel\_interaccion \\
SIMILAR\_TO & Movie → Movie & similitud, mismo\_genero, origen \\
\hline
\end{tabular}
...
\textbf{Integer} & 2024, 150 & Movie.año, User.edad \\
\textbf{Float} & 250.5, 0.85 & Movie.presupuesto, SIMILAR\_TO.similitud \\
\textbf{Boolean} & true, false & WATCHED.completado, BOOKMARKED.recordatorio \\
\textbf{List} & [``Oscar'', ``BAFTA''] & Actor.premios \\
\textbf{Date} & 2024-03-15 & User.fechaRegistro, WATCHED.fecha \\
\hline
\end{tabular}
...
Directores & 800 \\
\hline
\textbf{TOTAL DE NODOS} & \textbf{6,320} \\
\hline
\end{tabular}
\caption{Volumen de Datos Generados}
\end{table}

\textbf{Nota}: El total de nodos supera el mínimo requerido de 5,000.
...
\begin{itemize}
    ...
    \item \texttt{wrote_review.csv} - Relaciones WROTE\_REVIEW
    \item \textit{...y más}
\end{itemize}
