...
Contiene consultas utiles para analisis: productos por categoria,
estado de inventarios, proveedores top, ordenes pendientes y estado de transportes.
"""
...
class CypherQueries:
    ...
    def products_by_category(self, category: str) -> List[Dict]:
        def _query(tx):
            ...
            RETURN p.id_producto as id_producto, p.nombre as nombre,
                   p.precio as precio, p.perecedero as perecedero
            ORDER BY p.nombre
            """
            result = tx.run(cypher, category=category)
    ...
    def inventory_status_for_product(self, product_id: int) -> List[Dict]:
        def _query(tx):
            ...
            RETURN i.id_inventario as id_inventario, i.ubicacion as ubicacion,
                   i.cantidad as cantidad, i.capacidad_max as capacidad_max
            """
    ...
    def top_suppliers_by_rating(self, limit: int = 10) -> List[Dict]:
        def _query(tx):
            ...
            RETURN s.id_proveedor as id_proveedor, s.nombre as nombre,
                   s.pais as pais, s.rating as rating
    ...
    def pending_orders(self, limit: int = 50) -> List[Dict]:
        def _query(tx):
            ...
            RETURN o.id_orden as id_orden, o.fecha_orden as fecha_orden,
                   o.total as total, o.urgente as urgente
    ...
    def transport_status(self, limit: int = 50) -> List[Dict]:
        def _query(tx):
            ...
            RETURN t.id_transporte as id_transporte, t.tipo as tipo,
                   t.estado as estado, t.fecha_salida as fecha_salida
